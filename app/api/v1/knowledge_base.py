...
from app.schemas.knowledge_base import (
    DatasetDetail, KnowledgeBaseInput, KnowledgeBaseItem, 
    KnowledgeBaseResponse, KnowledgeBaseListResponse, 
    RetrievalModeSchema, RetrievalModel, RetrievalModelSchema, UpdateDatasetRequest
)
...
def list_datasets(
    ...
    try:
        ...
        rows, total = kb_repo.list_datasets(
            tenant_id=tenant_id,
            keyword=keyword,
            tag_ids=tag_ids or [],
            page=page,
...
def get_dataset_details(
    dataset_id: UUID = Path(..., description="KB ID (uuid)"),
    kb_repo = Depends(get_kb_repository),
    auth = Depends(get_current_user)
):
    tenant_id = auth["tenant_id"]

    row, tags, counts = kb_repo.get_dataset_detail(
...
def update_dataset(
    dataset_id: UUID = Path(..., description="KB ID"),
    body: UpdateDatasetRequest = Depends(),
    kb_repo = Depends(get_kb_repository),
    auth = Depends(get_current_user)
):
    tenant_id = auth["tenant_id"]
