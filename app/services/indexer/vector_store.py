...
class VectorRepository:
    ...
    def search(
        ...
        filters = self.build_kb_filter(kb_ids, tenant_id)
    ...
    def build_kb_filter(self, kb_ids: Optional[List[str]], tenant_id: Optional[str]) -> MetadataFilters:
        filters = []
        if tenant_id:
            filters.append(MetadataFilter(key="tenant_id", value=tenant_id, operator=FilterOperator.EQ))
        if kb_ids:
