...
class BotService:
    ...
    async def ask_bot(self, bot_id: str, query: str, tenant_id: str) -> str:
        ...
        result = self._call_llm(
            query=query,
            context=context,
            model=bot["config_model"]["model"],
            temperature=bot["config_model"]["temperature"]
        )
    ...
    async def _call_llm_stream(self, query: str, context: str, model: str, temperature: float) -> AsyncGenerator[str, None]:
        prompt = f"""
        Dựa trên thông tin sau, hãy trả lời câu hỏi một cách chính xác và rõ ràng.
        ===
        {context}
