...
from app.agent.config import BotRetrievalConfig, ChatConfigHelper
from app.core.factory import (
  get_embedding_model,
  get_graph_edge_repository,
  get_graph_entity_repository,
  get_vector_store,
)
...
class ChatRetrievalHelper:
  ...
  _embedding_cache: Dict[
    str, Tuple[Dict[Tuple[str, str, str], List[float]], float]
  ] = {}
  ...
  async def search_knowledge_bases(
    ...
    if not all_kb_ids:
      logger.warning(f"[Retrieval]: No Knowledge Bases found for tenant {tenant_id}")
      return []

    try:
      kb_configs_map = await self.kb_repo.get_retrieval_configs_by_ids(
        list(all_kb_ids), tenant_id, access_token=access_token
      )
    ...
    for kb_id, raw_config in kb_configs_map.items():
      try:
        ...
        parsed_configs[kb_id] = ParsedKBConfig(
          ...
          top_k=global_config.top_k * 2
          if global_config.rerank
          else global_config.top_k,
        )
      except Exception as e:
        logger.warning(f"[Retrieval]: Failed to parse config for KB {kb_id}: {e}")
    ...
    tasks = []
    valid_search_tasks = [
      (q, kb) for q, kbs in search_tasks for kb in kbs if kb in parsed_configs
    ]
    ...
    if not tasks:
      logger.warning("[Retrieval]: No valid search tasks. Returning empty results.")
      return []
    ...
    primary_query = (
      rerank_query if rerank_query else (search_tasks[0][0] if search_tasks else "")
    )
    ...
    for r in all_results:
      ...
      text = r.get("text", "")
      content_with_header = (
        f"Source (KB: {kb_name}) - File: {source_file}\nContent:\n{text}\n---"
      )

      doc = Document(page_content=content_with_header, metadata=meta)
    ...
    try:
      ...
      entity_ids = await entity_repo.get_entities_by_chunk_ids(
        retrieved_chunk_ids, access_token
      )
      if entity_ids:
        logger.info(
        ...
        if edges:
          ...
          if graph_relationships:
            ...
            graph_context_text = (
              ...
              + "\n".join(f"- {rel}" for rel in graph_relationships)
              + "\n---"
            )
      ...
      logger.error(f"[Retrieval]: Failed during Graph Traversal: {e}", exc_info=True)
  ...
  async def precompute_embeddings(
    ...
    cache_key_data = {
      ...
      "kb_configs": {
        k: (v.embedding_provider, v.embedding_model) for k, v in parsed_configs.items()
      },
    }
    ...
    ChatRetrievalHelper._embedding_cache[cache_key] = (embedding_cache, current_time)

    return embedding_cache

  async def list_kb_documents(
    self, kb_id: str, tenant_id: str, access_token: str = None
  ) -> List[dict]:
    """
    ...
    try:
      ...
      logger.exception(f"[Retrieval]: Failed to list documents for KB {kb_id}: {e}")
      return []
