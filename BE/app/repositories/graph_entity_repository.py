...
class GraphEntityRepository:
  ...
  async def get_entities_by_kb(
    self, kb_id: str, access_token: str = None
  ) -> List[Dict]:
  ...
  async def check_existing_entities(
    self, names: List[str], kb_id: str, access_token: str = None
  ) -> Dict[str, str]:
  ...
  async def get_entities_by_chunk_ids(
    self, chunk_ids: List[str], access_token: str = None
  ) -> List[str]:
    ...
    try:
      ...
      result = (
        await client.table(self.mentions_table)
        .select("entity_id")
