...
class ClassRepository:
  ...
  @classmethod
  def get_instance(cls) -> "ClassRepository":
  ...
  async def create_class(self, data: ClassCreateRequest) -> Optional[Dict[str, Any]]:
    try:
      from app.core.context import get_current_tenant_id
  ...
  async def enroll_student(self, class_id: str, user_id: str) -> bool:
    try:
      from app.core.context import get_current_tenant_id
      ...
      response = (
        ...
        .insert({"class_id": class_id, "user_id": user_id, "tenant_id": tenant_id})
        .execute()
      )
      return response.data[0] if response.data else None
    except Exception as e:
  ...
  async def get_student_enrollments(self, user_id: str) -> List[Dict[str, Any]]:
    try:
      from app.core.context import get_current_tenant_id
  ...
  async def get_classes_by_instructor(self, instructor_id: str) -> List[Dict[str, Any]]:
    try:
      from app.core.context import get_current_tenant_id
  ...
  async def get_stats(self) -> Dict[str, Any]:
    try:
      ...
      enrollments_count = (
        enrollments_res.count if hasattr(enrollments_res, "count") else 0
      )
  ...
  async def add_students_to_class(self, class_id: str, user_ids: List[str]) -> bool:
    try:
      from app.core.context import get_current_tenant_id
      ...
      await (
        ...
        .upsert(data, on_conflict="class_id,user_id")
        .execute()
      )
      return True
    except Exception as e:
  ...
  async def get_class_bots(self, class_id: str) -> List[Dict[str, Any]]:
    try:
      from app.core.context import get_current_tenant_id
  ...
  async def get_bots_by_ids(self, bot_ids: List[str]) -> List[Dict[str, Any]]:
    try:
      if not bot_ids:
        return []
  ...
  async def add_bots_to_class(self, class_id: str, bot_ids: List[str]) -> bool:
    try:
      from app.core.context import get_current_tenant_id
      ...
      existing_ids = (
        class_res.data.get("bot_ids", [])
        if (class_res.data and class_res.data.get("bot_ids"))
        else []
      )
  ...
  async def remove_bot_from_class(self, class_id: str, bot_id: str) -> bool:
    try:
      from app.core.context import get_current_tenant_id
  ...
  async def remove_student_from_class(self, class_id: str, user_id: str) -> bool:
    try:
      from app.core.context import get_current_tenant_id
