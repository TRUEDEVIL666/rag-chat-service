...
class SessionService:
  ...
  def get_total_sessions(self, tenant_id: str = None, access_token: str = None) -> int:
    return self.session_repo.get_total_sessions(tenant_id, access_token)

  def get_chat_messages(self, session_id: str, limit: int = 50, cursor_timestamp: int = None, sort_column: str = "created_at", sort_desc: bool = True, access_token: str = None) -> list[dict]:
    return self.chat_message_repo.get_messages_by_session(session_id, limit, cursor_timestamp, sort_column, sort_desc, access_token)
