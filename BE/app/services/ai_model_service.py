...
class AiModelService:
  ...
  def get_instance(cls) -> "AiModelService":
    if cls._instance is None:
      ...
      cls._instance = cls(repo=AiModelRepository.get_instance())
    return cls._instance

  def __init__(self, repo: AiModelRepository):
    self.repo = repo

  async def validate_model_type(self, provider_id: str, model_type: str):
    """
    ...
    allowed = capabilities.get(provider_name, ["chat", "embedding", "reranker"])
  ...
  async def get_models_by_provider(
    self, provider_id: str, model_type: Optional[str] = None
  ) -> List[Dict[str, Any]]:
    """
    Retrieves all active models for a specific provider, optionally filtered by type.
    """
    try:
      return await self.repo.get_models_by_provider(provider_id, model_type=model_type)
    except Exception as e:
      logger.error(
  ...
  async def list_models_by_type(self, model_type: str) -> List[Dict[str, Any]]:
    """
    Retrieves all active models of a specific type (e.g. 'chat', 'reranker').
    """
    try:
      return await self.repo.list_models_by_type(model_type)
    except Exception as e:
  ...
  async def list_all_models(self) -> List[Dict[str, Any]]:
    try:
      return await self.repo.list_all_models()
    except Exception as e:
  ...
  async def create_provider(self, provider_data: dict) -> Dict[str, Any]:
    try:
  ...
  async def update_provider(
    self, provider_id: str, update_data: dict
  ) -> Dict[str, Any]:
    try:
      return await self.repo.update_provider(provider_id, update_data)
    except Exception as e:
  ...
  async def delete_provider(self, provider_id: str) -> bool:
    try:
      await self.repo.delete_provider(provider_id)
      return True
    except Exception as e:
      logger.error(f"Error deleting provider {provider_id}: {e}")
      raise

  async def create_model(self, model_data: dict) -> Dict[str, Any]:
    try:
      await self.validate_model_type(
        model_data.get("provider_id"), model_data.get("model_type")
      )
  ...
  async def update_model(self, model_id: str, update_data: dict) -> Dict[str, Any]:
    try:
      if "provider_id" in update_data or "model_type" in update_data:
        # Get current model to fill in missing pieces for validation
        current_model = await self.get_model_by_id(model_id)
        ...
        await self.validate_model_type(p_id, m_type)

      if "provider_id" in update_data:
        update_data["provider_id"] = str(update_data["provider_id"])

      return await self.repo.update_model(model_id, update_data)
    except Exception as e:
  ...
  async def get_decrypted_key(self, provider_id: str) -> Optional[str]:
    try:
      return await self.repo.get_decrypted_key(provider_id)
    except Exception as e:
      logger.error(f"Error fetching decrypted key for provider {provider_id}: {e}")
      return None

  async def fetch_external_models(
    self, provider_id: str, model_type: str = None
  ) -> List[str]:
    """
    ...
    try:
      ...
      api_key = await self.repo.get_decrypted_key(provider_id)
      ...
      is_ollama_v1 = (
        "ollama" in provider_name and base_url and base_url.rstrip("/").endswith("/v1")
      )

      if "openai" in provider_name or is_ollama_v1:
        # Use OpenAI SDK - Synchronous call wrapped in thread for safety
        def _fetch_openai():
          target_url = base_url.rstrip("/") if base_url else "https://api.openai.com/v1"
      ...
      # --- Logic: Ollama (Native) ---
        ...
        target_url = base_url.rstrip("/") if base_url else "http://localhost:11434"
      ...
      logger.error(f"Unexpected error in fetch_external_models: {e}", exc_info=True)
