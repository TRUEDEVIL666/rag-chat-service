  ...
  try:
    ...
    if provider == "openai":
      ...
      lc_embed_model = GoogleGenerativeAIEmbeddings(
          model=f"models/{model_name}" if not model_name.startswith(
              "models/") else model_name,
    ...
    return LangchainEmbedding(langchain_embeddings=lc_embed_model, model_name=model_name)
