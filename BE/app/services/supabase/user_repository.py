...
class UserRepository:
	...
	def update_user_param(self, user_id: str, param_name: str, param_value: Any) -> dict | None:
		try:
			response = supabase.table(self.table_name).update({param_name: param_value}).eq("id", user_id).execute()
			if response.data:
	...
	def get_all_users_not_admin(self) -> list[dict] | None:
		try:
			response = supabase.table(self.table_name).select("*").neq("role", "admin").execute()
			if response.data:
	...
	def get_user_details(self, user_id: str) -> dict | None:
		try:
			response = supabase.from_(self.table_name).select("*").eq("id", user_id).single().execute()
			if response.data:
				return response.data
			return None
		except Exception as e:
			logger.exception(f"Failed to get user details from public.users for user_id: {user_id}")
			raise RuntimeError(f"Failed to get user details from public.users for user_id: {user_id}")
