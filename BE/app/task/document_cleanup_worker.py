...
def delete_document_background(self, document_id: str, tenant_id: str, kb_id: str, access_token: str):
  ...
  try:
    ...
    doc = _run_async(doc_repo.get_document_by_id(
      document_id, access_token=access_token))
    if not doc:
      logger.warning(
    ...
    try:
      _run_async(graph_chunk_repo.delete_by_document_id(document_id))
    ...
    _run_async(doc_repo.delete_document(
      document_id, access_token=access_token))
